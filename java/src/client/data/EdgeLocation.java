...
public class EdgeLocation {
    ...
    /**
     *
     * @param comparer the EdgeLocation to compare with
     * @pre comparer is not null
     * @post returns true if the HexLocation and the EdgeDirection are the
     * same. false otherwise.
     */
    public boolean equivalent(EdgeLocation comparer) {
        EdgeLocation thisOne = this.getNormalizedLocation();
        EdgeLocation realComparer = comparer.getNormalizedLocation();

        if (thisOne.getHexLoc().equivalent(realComparer.getHexLoc()) && thisOne.getDir().equals(realComparer.getDir())) {
            return true;
        }
        return false;
    }
