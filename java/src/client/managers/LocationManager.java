...
public class LocationManager {
	...
	/**
	 *
	 * @param locationToSettle
	 *            = the location the current player wants to build
	 * @param playerId
	 * @param firstRound
	 ...
	 *       unsettledLocations and the boolean canBeSettled of that Location is
			...
			if (location.getNormalizedLocation().equals(locationToSettle)) {
				...
				if (location.getCanBeSettled() && location.getWhoCanBuild().contains(playerId)) {
					...
					if (location.getNormalizedLocation().getDir() == VertexDirection.NE) {
						VertexLocation unusableLocation1 = new VertexLocation(
								location.getNormalizedLocation().getHexLoc(), VertexDirection.NW);
						VertexLocation unusableLocation2 = new VertexLocation(
								new HexLocation(location.getNormalizedLocation().getHexLoc().getX() + 1,
										location.getNormalizedLocation().getHexLoc().getY()),
								VertexDirection.NW);
						VertexLocation unusableLocation3 = new VertexLocation(
								new HexLocation(location.getNormalizedLocation().getHexLoc().getX() + 1,
										location.getNormalizedLocation().getHexLoc().getY() - 1),
								VertexDirection.NW);
						...
						EdgeLocation usableEdge2 = new EdgeLocation(location.getNormalizedLocation().getHexLoc(),
								EdgeDirection.NE);
						EdgeLocation usableEdge3 = new EdgeLocation(
								new HexLocation(location.getNormalizedLocation().getHexLoc().getX() + 1,
										location.getNormalizedLocation().getHexLoc().getY() - 1),
								EdgeDirection.NW);
						for (Edge edge : unsettledEdges) {
							if (edge.getEdgeLocation().equals(usableEdge1) || edge.getEdgeLocation().equals(usableEdge2)
					...
					if (location.getNormalizedLocation().getDir() == VertexDirection.NW) {
						VertexLocation unusableLocation1 = new VertexLocation(
								location.getNormalizedLocation().getHexLoc(), VertexDirection.NE);
						VertexLocation unusableLocation2 = new VertexLocation(
								new HexLocation(location.getNormalizedLocation().getHexLoc().getX() - 1,
										location.getNormalizedLocation().getHexLoc().getY()),
								VertexDirection.NE);
	...
	/**
	 *
	 * @param edgeLocationToSettle
	 *            = the edge the current player wants to build a road on
	 ...
	 *       unsettledEdges and the player's id is found in the whoCanBuild
			...
			if (edgeLocationToSettle.equals(edge.getEdgeLocation())) {
				if (edge.getWhoCanBuild().contains(playerId)) {
					...
					if (edge.getEdgeLocation().getDir() == EdgeDirection.N) {
						...
						EdgeLocation availableEdge1 = new EdgeLocation(edge.getEdgeLocation().getHexLoc(),
								EdgeDirection.NE);
						EdgeLocation availableEdge2 = new EdgeLocation(edge.getEdgeLocation().getHexLoc(),
								EdgeDirection.NW);
						EdgeLocation availableEdge3 = new EdgeLocation(
								new HexLocation(edge.getEdgeLocation().getHexLoc().getX() + 1,
										edge.getEdgeLocation().getHexLoc().getY() - 1),
								EdgeDirection.NW);
						EdgeLocation availableEdge4 = new EdgeLocation(
								new HexLocation(edge.getEdgeLocation().getHexLoc().getX() - 1,
										edge.getEdgeLocation().getHexLoc().getY()),
								EdgeDirection.NE);
					...
					if (edge.getEdgeLocation().getDir() == EdgeDirection.NE) {
						VertexLocation settleable1 = new VertexLocation(edge.getEdgeLocation().getHexLoc(),
								VertexDirection.NE);
						VertexLocation settleable2 = new VertexLocation(
								new HexLocation(edge.getEdgeLocation().getHexLoc().getX() + 1,
										edge.getEdgeLocation().getHexLoc().getY()),
								VertexDirection.NW);
						for (Location location : unsettledLocations) {
						...
						EdgeLocation availableEdge1 = new EdgeLocation(edge.getEdgeLocation().getHexLoc(),
								EdgeDirection.N);
						EdgeLocation availableEdge2 = new EdgeLocation(
								new HexLocation(edge.getEdgeLocation().getHexLoc().getX() + 1,
										edge.getEdgeLocation().getHexLoc().getY() - 1),
								EdgeDirection.NW);
						EdgeLocation availableEdge3 = new EdgeLocation(
	...
	/**
	 *
	 * @param locationToUpgrade
	 *            = the location the current player wants to upgrade the
	 *            settlement
	 ...
	 *       settledLocations, if the Location ownerID matches the player, and
	 *       if the Location's city boolean is false. Otherwise returns false.
	 */
	...
	/**
	 * @param triggeredHex
	 *            = the hex to reward the possible settlements and/or cities
	 ...
	 *       settled, and award the ownerID of those that are with the
		...
		for (Location location : settledLocations) {
			...
			for (HexLocation hexLocation : hexesAroundVertex) {
				if (hexLocation.equals(triggeredHex)) {
					// this is where the player gets added to a list to receive
					// a resource
	...
	public List<Port> getPortsPlayerHas(int playerId) {
		...
		for (VertexLocation settlementPlayerHas : settlementsPlayerHas) {
			List<HexLocation> neighboringHexes = getHexLocationsAroundVertexLocation(
					settlementPlayerHas.getNormalizedLocation());
