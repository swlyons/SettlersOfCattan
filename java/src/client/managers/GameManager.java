...
public class GameManager implements Serializable {
					...
					if (loc.getNormalizedLocation().toString().equals(settled.getNormalizedLocation().toString())) {
						...
						if (!(game.getPlayers().get(ownerId).getId() <= -1)) {
							...
							for (VertexObject city : game.getMap().getCities()) {
								if (Objects.equals(city.getLocation().getX(),
										settled.getNormalizedLocation().getHexLoc().getX())
										&& Objects.equals(city.getLocation().getY(),
												settled.getNormalizedLocation().getHexLoc().getY())) {
									isCity = true;
								}
							}
							switch (ownerId) {
	...
	public void placeSecondSettlement(VertexLocation v) {
		...
		for (Hex hex : mapManager.getHexList()) {
			if (hex.getLocation().toString().equals(neighbors.get(0).toString())
					|| hex.getLocation().toString().equals(neighbors.get(1).toString())
	...
	public boolean canPlaceRoad(EdgeLocation edgeLoc) {
		...
		if (locationManager.getSettledEdges().size() > 4 && locationManager.getSettledEdges().size() < 8) {
			...
			for (int i = 0; i < mySettledLocations.size(); i++) {
				...
				if (edgeLoc.toString().equals(e1.toString()) || edgeLoc.toString().equals(e2.toString())
						|| edgeLoc.toString().equals(e3.toString())) {
					return true;
				}
				return false;
			}
	...
	public boolean buildRoad(EdgeLocation edge) {
		...
		if (playerBank.getRoads() > 0 && playerBank.getResourcesCards().hasCardsAvailable(cost)) {
			...
			if (builtRoad) {
				...
				game.getMap().getRoads().add(edgeValue);
				ArrayList<EdgeValue> checkDuplicate = new ArrayList<EdgeValue>();
				for (EdgeValue e : game.getMap().getRoads()) {
					if (e.getOwner() == game.getTurnTracker().getCurrentTurn()) {
						if (checkDuplicate.size() == 0)
							checkDuplicate.add(e);
						else {
							for (EdgeValue dupe : checkDuplicate) {
								if(dupe.toString() != e.toString())
									checkDuplicate.add(e);
							}
						}
					}
				}
				resourceManager.getGameBanks().get(game.getTurnTracker().getCurrentTurn()).setRoads(15 - checkDuplicate.size());
	...
	public void endTurn() {
		 ...
		 * System.out.println(
		 * "Game Manager: Clear out whoCanBuild for Unsetteled Locations"); for
		 * (Location l : locationManager.getUnsettledLocations()) {
